...
class Ugd extends ugdController {
    ...
    public function __construct() {
        ...
        $this->home = base_url() . "emergency/ugd/";
    }
    ...
    public function profileMRH() {
        ...
        $data['dataUgd'] = $this->mdrughc->getTable('data_ugd', array('ID_RIWAYAT_RM'=>$id));
...
//        if(isset($dataUnit[1])){
            $data_queue = array(
                ...
                'sub_kia' => $this->input->post('subUnitPelayanan3')
            );
//        }
//        else {
//            $data_queue = array(
...
//        }
    ...
    public function updateRMHistory() {
        ...
        $dataUgd= array(
            'ID_RIWAYAT_RM'=>$this->input->post('id_rrm'),
            'UGD_LAKA_LANTAS'=>$this->input->post('UGD_LAKA_LANTAS'),
            'UGD_PENYAKIT_POTENSI_KLB'=>$this->input->post('UGD_PENYAKIT_POTENSI_KLB'),
            'UGD_SAKIT_LAIN'=>$this->input->post('UGD_SAKIT_LAIN'),
            'UGD_MATERNAL'=>$this->input->post('UGD_MATERNAL'),
            'UGD_NEONATAL'=>$this->input->post('UGD_NEONATAL'),
            'JENIS_LAKALANTAS'=>$this->input->post('JENIS_LAKALANTAS')
        );
        ...
        switch ($flagBtn) {
            case 1:
                ...
                $this->mdrughc->insert('data_ugd', $dataUgd);
                redirect($this->home);
                break;
            case 2:
...
//                if(isset($dataUnit[1])){
                    $data_queue = array(
                        ...
                        'FLAG_INTERN' => 1,
                        'waktu_antrian_unit' => $waktuantrian,
                        'sub_kia' => $this->input->post('subUnitPelayanan4')
                    );
//                }
//                else {
//                    $data_queue = array(
...
//        $ke= $this->mdrughc->getDataLengkapPenugasan(3, $idGedung);    //get id Apotik
    ...
    public function showTransResep($idTransaksi, $idRiwayatRm) {
        $queryTrans = $this->mdrughc->getSomeTransaksi($idTransaksi);
        $jenisTrans = $this->mdrughc->getSomeJenisTrans($queryTrans[0]['ID_JENISTRANSAKSI']);
        $rekamedik = $this->memergency->getRiwayatRMById($idRiwayatRm);
        ...
        for ($i = 0; $i < sizeof($queryDetailTrans); $i++):
            ...
            $queryDetailObatPasien = $this->memergency->getSomeObatPasien($idDetilTo);
    ...
    public function receipt($tipe,$param1=null,$param2=null){
        ...
        switch ($tipe) {
            case 'listPasien':
                ...
                break;
            case 'detailRiwayat':
                $array['namaPasien'] = $this->memergency->getTable('pasien', array('ID_PASIEN' => $param1));
                $url= $this->uri->segment(1, 0).'/'.$this->uri->segment(2, 0).'/'.$this->uri->segment(3, 0).'/'.$this->uri->segment(4, 0).'/'.$this->uri->segment(5, 0);
                $perPage= 10;
                $total_rows= $this->memergency->getRiwayatResepCount($param1);
                $pagination= $this->init_pagination($url,$total_rows,$perPage,6);
                $array['riwayat'] = $this->memergency->getRiwayatResep($param1, $this->uri->segment(6, 0), $perPage);
                $array["links"] = $this->pagination->create_links();
                $ke = $this->mdrughc->getDataLengkapPenugasan(3, $idGedung);    //get id Apotik
                $tebusan = $this->memergency->getTebusanObat($param1, $ke[0]['ID_UNIT']); 
                ...
                break;
            case 'addResep':
                $array['namaPasien']= $this->memergency->getTable('pasien', array('ID_PASIEN'=>$param2));
                $ke= $this->mdrughc->getDataLengkapPenugasan(3, $idGedung);    //get id Apotik
