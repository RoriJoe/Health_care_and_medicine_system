...
class mPatient extends CI_Model {
	...
	public function getEntryByRRM ($id_rrm) {
            $sql = "select temp.*, antrian_unit.WAKTU_ANTRIAN_UNIT from
                    (
                        select rm.id_rekammedik, rrm.id_riwayat_rm, p.id_pasien, p.noid_pasien, p.nama_pasien, 
                        p.alamat_pasien, p.gender_pasien, floor((datediff(curdate(),p.tgl_lahir_pasien)*12) / 365) as umur , 
                        sumber_pembayaran_pasien.NAMA_SUMBER_PEMBAYARAN, sumber_pembayaran_pasien.ID_SUMBER, TANGGAL_RIWAYAT_RM
                        from riwayat_rm rrm 
