<?php

if (!defined('BASEPATH'))
    exit('No direct script access allowed');

class mMRHistory extends CI_Model {

    public function __construct() {
        parent::__construct();
    }

    public function insertNewEntry($idrm, $data) {
		$row = $this->getMRHByIdMR($idrm, 100);
		$data['session_kunjungan'] = $row[0]['SESSION_KUNJUNGAN'];
		$idrrm; $jamvisite;
		foreach ($row as $t) {
			$idrrm = $t['ID_RIWAYAT_RM'];
			$jamvisite = $t['WAKTU_PERIKSA'];
			break;
		}
		
		if (empty($jamvisite)) {
			// update 
			// $idrrm = $row[0]['ID_RIWAYAT_RM'];			
			return $this->updateEntry($data, $idrrm);
		}
		else {
			$this->db->insert('riwayat_rm', $data);
			return $this->db->insert_id();			
		}
    }
	
	function removeEntry ($data) {
		$sql1 = 'set foreign_key_checks = 0';
		$sql2 = 'DELETE FROM antrian_unit WHERE ID_ANTRIAN_UNIT = '.$data;
		$sql3 = 'set foreign_key_checks = 1';
	
		$this->db->query ($sql1);
		$this->db->query ($sql2);
		$this->db->query ($sql3);
	}
	
	// function updateEntry ($idrrm, $data) {
		// $this->db->where ('ID_RIWAYAT_RM', $idrrm);
		// $this->db->update('riwayat_rm', $data);
		// $affectedRows = $this->db->affected_rows();		
		// return $affectedRows['ID_RIWAYAT_RM'];
	// }
	
	public function getProfilMRH ($id) {
		$sql = 'SELECT pasien.* FROM riwayat_rm
				left join rekam_medik
				on riwayat_rm.ID_REKAMMEDIK = rekam_medik.ID_REKAMMEDIK
				left join pasien
				on rekam_medik.ID_PASIEN = pasien.ID_PASIEN
				where riwayat_rm.ID_RIWAYAT_RM = '.$id;
		$query = $this->db->query ($sql);
		if ($query->num_rows() > 0)
            return $query->result_array();
        else
            return null;
	}

    public function getMRHById($id) {
		$sql = 'SELECT riwayat_rm.*, sumber_pembayaran_pasien.NAMA_SUMBER_PEMBAYARAN AS SUMBER_PEMBAYARAN FROM riwayat_rm 
		LEFT JOIN sumber_pembayaran_pasien ON sumber_pembayaran_pasien.ID_SUMBER = riwayat_rm.ID_SUMBER
		WHERE riwayat_rm.id_riwayat_rm = '.$id;
		
        $query = $this->db->query ($sql);
		if ($query->num_rows() > 0)
            return $query->result_array();
        else
            return null;
    }

	public function getMRHByIdMR($id, $limit) {
		$sql = 'SELECT pasien.NAMA_PASIEN, riwayat_rm.TANGGAL_RIWAYAT_RM AS TANGGAL_PERIKSA, riwayat_rm.ID_RIWAYAT_RM, riwayat_rm.RI_JAM_VISITE AS WAKTU_PERIKSA, riwayat_rm.SESSION_KUNJUNGAN
		FROM riwayat_rm
		LEFT JOIN rekam_medik ON rekam_medik.ID_REKAMMEDIK = riwayat_rm.ID_REKAMMEDIK
		LEFT JOIN pasien ON rekam_medik.ID_PASIEN = pasien.ID_PASIEN
		LEFT JOIN data_rawat_inap ON data_rawat_inap.ID_RAWAT_INAP = riwayat_rm.ID_RAWAT_INAP
		LEFT JOIN tempat_tidur ON data_rawat_inap.ID_TEMPAT_TIDUR = tempat_tidur.ID_TEMPAT_TIDUR
		LEFT JOIN ruangan_ri ON ruangan_ri.ID_RUANGAN_RI = tempat_tidur.ID_RUANGAN_RI
		WHERE rekam_medik.ID_REKAMMEDIK = ' . $id . ' AND ruangan_ri.ID_UNIT = ' . $this->session->userdata['telah_masuk']['idunit'] . ' 
		ORDER BY riwayat_rm.ID_RIWAYAT_RM DESC ';
		
		// $sql = 'select pasien.NAMA_PASIEN, riwayat_rm.TANGGAL_RIWAYAT_RM as 	
				// TANGGAL_PERIKSA, riwayat_rm.ID_RIWAYAT_RM, riwayat_rm.RI_JAM_VISITE as WAKTU_PERIKSA
				// from rekam_medik
				// LEFT JOIN pasien ON rekam_medik.ID_PASIEN = pasien.ID_PASIEN
				// LEFT JOIN riwayat_rm ON rekam_medik.ID_REKAMMEDIK = riwayat_rm.ID_REKAMMEDIK
				// LEFT JOIN antrian_unit ON antrian_unit.ID_RIWAYAT_RM = riwayat_rm.ID_RIWAYAT_RM
				// WHERE rekam_medik.ID_REKAMMEDIK = ' . $id . ' AND antrian_unit.FLAG_ANTRIAN_UNIT = 1
				// AND antrian_unit.ID_UNIT = ' . $this->session->userdata['telah_masuk']['idunit'] . '
				// ORDER BY riwayat_rm.ID_RIWAYAT_RM DESC ';
		// $sql = 'select pasien.NAMA_PASIEN, riwayat_rm.TANGGAL_RIWAYAT_RM as 	
				// TANGGAL_KUNJUNGAN, riwayat_rm.ID_RIWAYAT_RM
				// from rekam_medik
				// left join pasien on rekam_medik.ID_PASIEN = pasien.ID_PASIEN
				// left join riwayat_rm on rekam_medik.ID_REKAMMEDIK = riwayat_rm.ID_REKAMMEDIK
				// where rekam_medik.ID_REKAMMEDIK = ' . $id . '
				// ORDER BY riwayat_rm.ID_RIWAYAT_RM DESC ';
					
		if($limit != null){
			$sql .= 'LIMIT '.$limit;
        }

        $query = $this->db->query($sql);
        if ($query->num_rows() > 0)
            return $query->result_array();
        else
            return null;
    }
	
    public function updateEntry($data, $id) {
		$this->db->query ('SET foreign_key_checks = 0');
		
        $this->db->where('id_riwayat_rm', $id);
        if ($this->db->update('riwayat_rm', $data)) {
			$this->db->query ('SET foreign_key_checks = 1');
			$affectedRows = $this->db->affected_rows();		
			return $affectedRows['id_riwayat_rm'];
        } else {
			$this->db->query ('SET foreign_key_checks = 1');
            return null;
		}
    }

    public function getHistoryRRM($id) {
        $sql = 'select rm.id_rekammedik, rm.noid_rekammedik, rrm.id_riwayat_rm, p.id_pasien, p.nama_pasien,p.alamat_pasien, rrm.beratbadan_pasien, rrm.tinggibadan_pasien, rrm.sistol_pasien, rrm.diastol_pasien, rrm.tanggal_riwayat_rm, au.id_unit from antrian_unit au left join riwayat_rm rrm
				on au.id_riwayat_rm = rrm.id_riwayat_rm left join rekam_medik rm
				on rm.id_rekammedik = rrm.id_rekammedik left join pasien p on p.id_pasien = rm.id_pasien where au.flag_antrian_unit = 1 and au.id_unit=' . $id;
        $query = $this->db->query($sql);
        if ($query->num_rows() > 0)
            return $query->result_array();
        else
            return null;
    }
	
	public function getResepPasien($idUnit) {
        $sql = 'SELECT rm.id_rekammedik, rm.noid_rekammedik, rrm.id_riwayat_rm, p.id_pasien
                , p.nama_pasien,p.alamat_pasien, rrm.beratbadan_pasien, rrm.tinggibadan_pasien
                , rrm.sistol_pasien, rrm.diastol_pasien, rrm.tanggal_riwayat_rm, au.id_unit 
                , noid_pasien, gender_pasien, telepon_pasien, gol_darah_pasien, agama_pasien
                FROM antrian_unit au 
                LEFT JOIN riwayat_rm rrm ON au.id_riwayat_rm = rrm.id_riwayat_rm 
                LEFT JOIN rekam_medik rm ON rm.id_rekammedik = rrm.id_rekammedik 
                LEFT JOIN pasien p ON p.id_pasien = rm.id_pasien 
                WHERE au.flag_antrian_unit = 1 AND au.id_unit=?
                GROUP BY rm.id_rekammedik
                ORDER BY rrm.id_riwayat_rm DESC';
        $query = $this->db->query($sql, array($idUnit));
        if ($query->num_rows() > 0)
            return $query->result_array();
        else
            return null;
    }
	
	public function getMedRecord ($id_unit) {
		$sql = 'select rm.id_rekammedik, rm.noid_rekammedik, p.id_pasien, 		
			p.nama_pasien,p.alamat_pasien from antrian_unit au left join riwayat_rm rrm
			on au.id_riwayat_rm = rrm.id_riwayat_rm left join rekam_medik rm
			on rm.id_rekammedik = rrm.id_rekammedik left join pasien p on p.id_pasien = rm.id_pasien where au.flag_antrian_unit = 1 and au.id_unit= '.$id_unit.'
			group by rm.id_rekammedik';
		$query = $this->db->query($sql);
        if ($query->num_rows() > 0)
            return $query->result_array();
        else
            return null;
	}

}
