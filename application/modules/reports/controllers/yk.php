...
class Yk extends ykController {
	...
	function drugUsed()
    {
		...
		$data['allDrugUsed'] = $this->drugused_model->getAllUsedDrugByDate($idPuskesmas,$unit,$from, $till);
	...
	function drugStockCard()
    {
		...
		{
			$idPuskesmas = $this->session->userdata['telah_masuk']['idgedung'];
			$idUnit = $this->unit_model->getGOPID();
...
//        $this->script_footer = 'lay-scripts/footer_grafik';
    ...
    {
		$form_data = $this->input->post(null, true);
		if($this->session->userdata['telah_masuk']['idha'] != 19 && $this->session->userdata['telah_masuk']['idha'] != 15)
		{
