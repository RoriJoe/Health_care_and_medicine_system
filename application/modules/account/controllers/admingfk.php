...
class Admingfk extends agfkController {
	...
	function addAccountWTask()
    {
        ...
        $IdAkun = $this->aaccount->findId($this->input->post('inputNoid'))[0]['ID_AKUN']; 
		for($i = 1; $i<=$numRow;$i++)
		{
	...
	function addTask()
    {
        ...
        $IdAkun = $this->aaccount->findId($this->input->post('dataAkun'))[0]['ID_AKUN']; 
		...
		for($i = 1; $i<=$numRow;$i++)
		{
			if(isset($form_data['unit'.$i]))
			{
				...
				else
				{
					...
					$this->assignment->deactivateAssignment($IdAkun,$dataPenugasan['id_hakakses'],$form_data['unit'.$i],$dataPenugasan);
				}
			}
		}
