...
class Ap extends apController {
	...
	function addAccountWTask()
    {
		...
		for($i = 1; $i<=$numRow;$i++)
		{
			...
			{
				$arrIndex = $i;
				$unitName = $this->unit_model->getUnitById($form_data['unit'.$i]);
				if($unitName[0]['NAMA_UNIT'] != NULL || $unitName[0]['NAMA_UNIT'] != ''){
					if(strpos(strtoupper ($unitName[0]['NAMA_UNIT']),'PUSTU') !== false || strpos(strtoupper ($unitName[0]['NAMA_UNIT']),'POLINDES') !== false || strpos(strtoupper ($unitName[0]['NAMA_UNIT']),'PONKESDES') !== false)
					$idHa = $this->access_model->getHAByUnit('PUSTU');
					else
					$idHa = $this->access_model->getHAByUnit($unitName[0]['NAMA_UNIT']);
					$dataPenugasan = array (
						'id_akun' => $IdAkun,
						'id_unit' => $form_data['unit'.$i],
						'id_hakakses' => $idHa[0]['ID_HAKAKSES'],
						'flag_active_penugasan' => 1
					);
					if($this->assignment->existChecker($IdAkun,$form_data['unit'.$i],$dataPenugasan['id_hakakses']) == false)
    ...
    function updateAccount()
    {
        ...
        $data['allAssignment'] = $this->assignment->getAssignmentByIdAkun($id);
		else
		$data['allAssignment'] = $this->assignment->getAssignmentByIdAkunForAP($id);
    ...
    {
        $form_data = $this->input->post(null, true);
		$passAcc = $this->aaccount->selectById($form_data['selectedAkun']);
        if ($form_data == null ) redirect( base_url().'index.php/Account', 'refresh');
        else
        {
            $data = array (
		...
		if($this->session->userdata['telah_masuk']['idha'] == 19)
        $data['allAssignment'] = $this->assignment->getAssignmentByIdAkun($IdAkun);
		else
		$data['allAssignment'] = $this->assignment->getAssignmentByIdAkunForAP($IdAkun);
		$diffArray = array();
		for($i = 1; $i<=$numRow;$i++)
		{
			if(isset($form_data['unit'.$i]))
			{
				...
				if(!isset($_POST['unit'.$i]))
					break;
				$infosplit = explode("_",$form_data['unit'.$i]);
				$alreadyHA = '';
				//echo $infosplit[1];
				if(isset($infosplit[1]))
				$alreadyHA = $infosplit[1];
				$unitName = $this->unit_model->getUnitById($infosplit[0]);
				if(strpos(strtoupper ($unitName[0]['NAMA_UNIT']),'PUSTU') !== false || strpos(strtoupper ($unitName[0]['NAMA_UNIT']),'POLINDES') !== false || strpos(strtoupper ($unitName[0]['NAMA_UNIT']),'PONKESDES') !== false)
				$idHa = $this->access_model->getHAByUnit('PUSTU');
				else
				$idHa = $this->access_model->getHAByUnit($unitName[0]['NAMA_UNIT']);
				if($alreadyHA != '')
				{
					...
					'id_akun' => $IdAkun,
					'id_unit' => $infosplit[0],
					'id_hakakses' => $alreadyHA,
					'flag_active_penugasan' => 1
					);
					$diffArray[$arrIndex-1] = $dataPenugasan;
					if($this->assignment->existChecker($IdAkun,$infosplit[0],$alreadyHA) == false)
					{
						$this->assignment->insertNewEntry($dataPenugasan);
					}
					else
					{
						...
						$this->assignment->deactivateAssignment($IdAkun,$alreadyHA,$infosplit[0],$dataPenugasan);
					}
				}
				else
				{
					...
					$diffArray[$arrIndex-1] = $dataPenugasan;
					if($this->assignment->existChecker($IdAkun,$form_data['unit'.$i],$idHa[0]['ID_HAKAKSES']) == false)
					{
						$this->assignment->insertNewEntry($dataPenugasan);
					}
					else
					{
						...
						$this->assignment->deactivateAssignment($IdAkun,$idHa[0]['ID_HAKAKSES'],$form_data['unit'.$i],$dataPenugasan);
					}
				}
			}
		}
