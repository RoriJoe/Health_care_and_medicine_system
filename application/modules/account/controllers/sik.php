...
class Sik extends sikController {
		...
		for($i = 1; $i<=$numRow;$i++)
		{
			if(isset($form_data['unit'.$i]))
			{
				$dataPenugasan = array (
					...
					'id_hakakses' => $form_data['ha'.$i]
				);
				
				if($this->assignment->existChecker($IdAkun,$form_data['unit'.$i],$form_data['ha'.$i]) == false)
	...
	function addTask()
    {
		...
		redirect( base_url().$this->uri->segment(1, 0).'/'.$this->uri->segment(2, 0).'/addAccount', 'refresh');
    }
