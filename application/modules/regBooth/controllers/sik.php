...
class Sik extends sikController {
	...
	public function queue () {
		...
		$this->display('queueKd', $data);
	}
	
	public function queueRI () {
		$data['units'] = $this->Unit_model->getIDRi ();
	...
	function showQueueUnit () {
		...
		$result = $this->status_queue_model->getQueueByUnit ($unit,$puskesmas,$tanggal);
		if ($result) 
	...
	function showRI () {
		$puskesmas = $this->input->post('puskesmas');
		$tanggal = $this->input->post('tanggal');
		$result = $this->ri_model->getRIPatient ($puskesmas,$tanggal);
		if ($result) 
		echo $this->getPivotAntrian ($result);
		else echo "[[\"NIK\",\"NAMA PASIEN\", \"UMUR\", \"JENIS KELAMIN\", \"TANGGAL MASUK\"], [\"Kosong\", \"Kosong\", \"Kosong\", \"Kosong\", \"Kosong\"]]";
	}
