...
class mDrugsTransaction extends CI_Model {
    ...
    function getHCRequest ($id) {
    	$sql = 'select transaksi_obat.ID_TRANSAKSI from transaksi_obat
                left join unit  on transaksi_obat.TRANSAKSI_UNIT_DARI = unit.ID_UNIT
                left join gedung on unit.ID_GEDUNG = gedung.ID_GEDUNG and gedung.ID_GEDUNG = '.$id.'
    ...
    function getPendingRequest (){
    	$sql = 'select transaksi_obat.ID_TRANSAKSI, transaksi_obat.TANGGAL_TRANSAKSI, 
                unit.ID_UNIT, unit.NAMA_UNIT, gedung.NAMA_GEDUNG 
                from transaksi_obat 
                left join unit on transaksi_obat.transaksi_unit_dari = unit.id_unit
                left join gedung on unit.id_gedung = gedung.id_gedung
                where transaksi_obat.id_jenistransaksi = 14 and transaksi_obat.transaksi_unit_ke = '.$this->session->userdata['telah_masuk']['idunit'].
    ...
    function insertNewEntryMinus ($data, $data_detail) {
        ...
        if ($latest_id != -1) {
            ...
            //minus gfk
            $this->mDrugsDetailTrans->insertManyEntryMinus ($data_detail);
        }  
        return $latest_id;
    }
