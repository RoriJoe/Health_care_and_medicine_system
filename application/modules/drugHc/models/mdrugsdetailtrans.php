...
class mDrugsDetailTrans extends CI_Model {
    ...
    function getEntryById($idTrans) {
        $idunitini = $this->session->userdata['telah_masuk']['idunit'];

        $sql = 'SELECT trans.*, terkini.KODE_OBAT, terkini.SATUAN, terkini.TOTAL FROM
		...
		WHERE detil_transaksi_obat.id_transaksi = ' . $idTrans . '
    ...
    function getEntryByIdPerBatch($id) {
        ...
        $sql = 'SELECT trans.*, terkini.KODE_OBAT, terkini.SATUAN, terkini.STOK_OBAT_SEKARANG FROM
    ...
    function showDetailTransById($id) {
        $sql = 'SELECT detil_transaksi_obat.*, sumber_anggaran_obat.NAMA_SUMBER_ANGGARAN_OBAT, 
                ...
                WHERE detil_transaksi_obat.ID_TRANSAKSI = ' . $id . ' ORDER BY detil_transaksi_obat.ID_DETIL_TO DESC';
        $query = $this->db->query($sql);
        if ($query->num_rows > 0)
            return $query->result_array();
        else
