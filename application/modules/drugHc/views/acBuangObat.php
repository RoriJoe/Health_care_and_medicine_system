            ...
            <div class="panel panel-primary">
                ...
                <div class="panel-body">
                    ...
                    <form class="form-horizontal bucket-form" method="post" action="<?= base_url().$this->uri->segment(1, 0).'/'.$this->uri->segment(2, 0).'/addRequest' ?>">
                        <div class="form-group">
                            <label class="col-sm-3 control-label">Tanggal Pembuangan Obat</label>
                            <div class="col-sm-3">
...
<script type="text/javascript">
    ...
    function selectObat(detil_id, id_obat, batch, nama_obat, expObat, pesan, jmlObat, satObat){
        if(Number(jmlObat)<Number(pesan)){
            alert("Pemilihan Obat Melebihi Stok");
        }
        else {
...
//        alert(kapsul.teksnya.tanda);
        $.ajax({
            type : "POST",
            url : '<?php echo base_url().$this->uri->segment(1, 0).'/'.$this->uri->segment(2, 0).'/searchObat/'.$dari_id.'/retur';  //id unit operator gfk ?>',
            data: kapsul,
...
//                alert(dataCheck);
                ...
                setupPivot({
                    json : jso,
                    fields : fields,
                    rowLabels : ["NOMOR","NOMOR BATCH", "NAMA OBAT","STOK","SATUAN","EXPIRED DATE", "KELOLA"]
                })
