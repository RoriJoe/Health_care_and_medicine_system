...
class Ogfk extends ogfkController {
	...
	public function updateDrugsGFK (){
		...
		// $dataDetail = array (
				// 'expired_date' => $form_data['selectedExpiredObat'],
				// 'nomor_batch' => $form_data['selectedBatchObat']
		// );
		
		// $this->mDrugsDetailTrans->hihi($form_data['selectedIdObat'], $dataDetail);
	...
	private function getPivot($data) {
        ...
        foreach ($data as $value) {
			...
			$header .= ',"<a style=\"color: white\" type=\"button\" data-toggle=\"modal\" href=\"#editModal\" class=\"btn btn-xs btn-info\" onclick=\"drugsEditGFK(\''.$value['KODE_OBAT'].'\',\''.$value['NAMA_OBAT'].'\',\''.$value['SATUAN'].'\',\''.$value['JML_OBAT_MIN'].'\')\"><i class=\"fa fa-edit\"></i></a>"';
			...
			$header .= ',"<a style=\"color: white\" type=\"button\" class=\"btn btn-xs btn-danger\"  onclick=\"drugsThrowGFK(\''.$value['ID_OBAT'].'\',\''.$value['NAMA_OBAT'].'\',\''.$value['NOMOR_BATCH'].'\')\"><i class=\"fa fa-ban\"></i></a>"';
	...
	function addThrowDrugsGFK () {
        ...
        $haystack = $this->session->userdata('drugs_throwed_detail');
        //if ($haystack) {
        $flag = true;
        if ($haystack != null) {
        ...
        //}
        $this->session->set_userdata('drugs_throwed_detail', $haystack);
        echo json_encode($this->session->userdata('drugs_throwed_detail'));
	}
	
	function removeDrugsDetailPembuangan () {
		$form_data = $this->input->post(null, true);
		$haystack = $this->session->userdata('drugs_throwed_detail');
        ...
        if ($haystack) {
            for ($i = 0; $i < count($haystack); $i++) {
                if (in_array($form_data['id_obat'], $haystack[$i], true) && in_array($form_data['nomor_batch'], $haystack[$i], true)) {
					unset($haystack[$i]);
                    $this->session->set_userdata('drugs_throwed_detail', $haystack);  break;
                }
            }
        }
	}
	...
	function addDrugsThrowed () {
        ...
        $result = $this->mDrugsTransaction->insertNewEntryMinus($data, $this->session->userdata('drugs_throwed_detail'));
	...
	private function getPivotOK($data) {
        ...
        foreach ($data as $value) {
			...
			$header .= ',"<a style=\"color: white\" type=\"button\" data-toggle=\"modal\" href=\"#addStocksModal\" class=\"btn btn-xs btn-success\" onclick=\"toAddStocksModal(\''.$value['ID_OBAT'].'\',\''.$value['NAMA_OBAT'].'\',\''.$value['TOTAL'].'\')\"><i class=\"fa fa-check\"></i></a>"';
            // $header .= "]";
			...
			foreach ($form_data as $key => $value) {
				if (strpos($key, "obat") !== false) {
					...
					if (strcmp($detail[$i]['ID_OBAT'],$idobat) == 0 && strcmp($detail[$i]['NOMOR_BATCH'],$nomorbatch) == 0){			
						$detail[$i]['STOK_OBAT_SEKARANG'] = (string) $jmlobat;
						$detail[$i]['ID_UNIT'] = $this->session->userdata['telah_masuk']['idunit'];
						$idobat =''; $nomorbatch=''; $jmlobat='';
					}
					else  {	
						$detail[$i]['STOK_OBAT_SEKARANG'] = (string) $detail[$i]['JUMLAH_OBAT'];
						$detail[$i]['ID_UNIT'] = $this->session->userdata['telah_masuk']['idunit'];
					}
				}
			}				
