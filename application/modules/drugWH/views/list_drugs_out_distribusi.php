...
function showDetailRequest (value, tujuan, tanggalkirim) {
	...
	$.ajax({
        type: "POST",
        url: "<?php echo base_url().$this->uri->segment(1).'/'.$this->uri->segment(2); ?>/showDetailRequestPerBatch",
        data: {id : value},
        success: function(data){   
			...
			content += '<table class="table table-striped table-comparison table-responsive"><thead><tr><th>KODE OBAT</th><th>NAMA OBAT</th><th>SATUAN</th><th>NOMOR BATCH</th><th>JUMLAH </th></thead><tbody id="bodyChoosed">';
				...
				content += '</tbody></table>';				
				content += '<form class="pull-right" method="post" action="<?php echo base_url().$this->uri->segment(1).'/'.$this->uri->segment(2); ?>/setujuiDistribusi"><input type="hidden" id="inputTransaksi" name ="inputTransaksi" value="'+transid+'"><input type="submit" class="btn btn-primary" value="Setujui Pengiriman">';
...
function hapusJumlah (idobat, idtrans) {
	...
	$.ajax({
        ...
        success: function(ret){
			// alert (ret);
			$('#bodyChoosed').find('#obat-'+ret).remove();
		},
		error: function(){}
	});
}
